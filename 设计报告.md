---
title: 基于Sobel算子的Canny边缘检测算法在车牌识别中的应用案例研究
author: Tiantian Li
---
...
<img src="./out-1.png" alt="out-1" style="zoom:50%;" />

<img src="./out-2.png" alt="out-2" style="zoom:50%;" />

<img src="./out-3.png" alt="out-3" style="zoom:50%;" />

<img src="./out-4.png" alt="out-4" style="zoom:50%;" />
...
- numpy（对图像信息进行操作）
- opencv_python（读取、写入图像，膨胀与腐蚀，轮廓检测）注：OpenCV的引入与Canny算法本身并无关系，主要用于读取写入和后续车牌检测。
- scipy（卷积操作）

在依赖安装完备的情况下：

- 执行`python my_canny.py <path-to-image> <low_threshold> <high_threshold>`将在当前目录生成`out.png`为边缘检测的结果。
- 执行`python car_plate.py <input-path> <output-path>`将在`output-path`生成车牌检测的结果。

## 文件结构

- `images`下为各输入样例，包括前文用到的莱娜图和各种车牌图。
- `images/lena`下为遍历高阈值对莱娜图进行边缘检测得到的结果。
- `./`下有若干`out-*.png`为车牌检测的结果。

## 总结

本文主要通过使用Python编程语言实现了经典的Canny边缘检测算法，并尝试将该算法应用于车牌识别任务。研究的主要目标是开发一种有效的方法来提取图像中的车牌边缘，以辅助车牌的定位和识别。

总结起来，本文成功实现了Canny边缘检测算法的Python版本，并尝试将其应用于车牌识别。通过这项研究，我们展示了Canny算法在车牌识别领域的潜力。未来的工作可以进一步改进算法的性能和鲁棒性，并探索更多的车牌识别方法和技术。
